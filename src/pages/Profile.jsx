  ...
  return (
    ...
    {/* Shows the logged-in user's identity and supplies the owner-only edit action. */}
      <ProfileHeader
        ...
        action={
          <button type="button"
          className="rounded-full bg-(--accent) px-5 py-2.5 text-sm font-bold text-white transition hover:brightness-110 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--accent)"
      ...
      {isEditing && (
        <section className="mt-5 rounded-2xl border border-(--accent-border) bg-[#211b31] p-5 sm:p-6">
          ...
          <label className="mt-5 block">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-(--text-h) outline-none transition placeholder:text-(--text) focus:border-(--accent)"
          ...
          <label className="mt-4 block">
            ...
            <input
              ...
              minLength={3}
              maxLength={20}
              className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-(--text-h) outline-none transition focus:border-(--accent) "
            />
          </label>
          ...
          <fieldset className="mt-5">
            ...
            <div className="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm font-semibold text-(--text-h) transition hover:border-(--accent-border)">
                <input
                  type="radio"
          ...
          <div className="mt-5">
            ...
            <button
              type="button"
              disabled
              className="mt-2 w-full cursor-not-allowed rounded-xl border border-dashed border-white/15 bg-black/10 px-4 py-3 text-sm font-semibold text-(--text) opacity-70"
            >
          ...
          <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <button
              ...
              className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold text-(--text-h) transition hover:bg-white/5"
            >
              Cancel
            </button>
            ...
            <button
              ...
              disabled={isSaving}
              className="rounded-xl bg-(--accent) px-4 py-3 text-sm font-bold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
            >
          ...
          {saveMessage && (
            <p
              role="status"
              className="mt-3 rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-center text-sm text-emerald-300"
            >
              {saveMessage}
            </p>
          )}
      ...
      <section className="mt-5 rounded-2xl border border-white/10 bg-[#211b31] p-5 sm:p-6">
        <div>
          <h2 className="text-xl font-extrabold text-(--text-h) sm:text-2xl">
            Event history
          </h2>
        ...
        {eventsLoading ? (
          <p className="mt-5 rounded-xl border border-dashed border-white/15 bg-black/10 px-4 py-8 text-center text-sm text-(--text)">
      ...
      <section className="mt-5 rounded-2xl border border-white/10 bg-[#211b31] p-5 sm:p-6">
        ...
        <button
          type="button"
          onClick={getLocation}
          className="mt-5 w-full rounded-xl border border-(--accent-border) bg-(--accent-bg) px-4 py-3 text-sm font-bold text-(--text-h) transition hover:border-(--accent) hover:bg-(--accent) hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--accent)"
        >
      ...
      {showUsernameWarning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <section
            ...
            className="w-full max-w-md rounded-2xl border border-(--accent-border) bg-[#211b31] p-5 text-center shadow-2xl sm:p-6"
          >
            <h2
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <button
                type="button"
                onClick={handleSaveProfile}
                className="rounded-xl bg-(--accent) px-4 py-3 text-sm font-bold text-white transition hover:brightness-110"
              >
                Change username
              </button>
            </div>
