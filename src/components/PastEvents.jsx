...
function PastEvents({ pastEvents }) {
  ...
  const paginatedEvents = pastEvents.slice(start, start + itemsPerPage);
  const maxPages = Math.ceil(pastEvents.length / itemsPerPage);
  ...
  if (pastEvents.length === 0) {
    return <div><h2>Past Events</h2><p>No past events yet.</p></div>;
  }
